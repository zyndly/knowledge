  ...
  useEffect(() => {
    ...
    const handleObjectModified = (e: any) => {
      ...
      if (obj.data?.type === "arrow" && obj instanceof fabric.Group) {
        const line = obj
          .getObjects()
          .find((o): o is fabric.Line => o.type === "line");
        ...
        updates.endX = p2.x;
        updates.endY = p2.y;
        updates.rotation = 0; // Rotation is handled by the group, so reset it to 0 for storage
      }
      // Handle text
      if (obj.data?.type === "text" && obj.type === "i-text") {
        const textObj = obj as fabric.IText;

        const scaleY = textObj.scaleY ?? 1;

        // Convert scale into real font size
        if (scaleY !== 1) {
          textObj.fontSize = (textObj.fontSize || 20) * scaleY;
      ...
      switch (e.key.toLowerCase()) {
        ...
        case "backspace":
          if (selectedAnnotationId) {
            ...
            useEditorStore
              .getState()
              .deleteAnnotation(step._id, selectedAnnotationId);
          }
          break;
      }
    };
